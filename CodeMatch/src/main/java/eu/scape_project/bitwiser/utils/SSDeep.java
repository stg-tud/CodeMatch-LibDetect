 ...
 *
 * <p>
 * A Java version of the ssdeep algorithm, based on the fuzzy.c source code,
 * taken from version 2.6 of the ssdeep package.
 * 
 * <p>
...
public class SSDeep {
	...
	// / The longest possible length for a fuzzy hash signature (without the
	// filename)
	...
	/*
	 * a rolling hash, based on the Adler checksum. By using a rolling hash we
	 * can perform auto resynchronisation after inserts/deletes
	 * 
	 * internally, h1 is the sum of the bytes in the window and h2 is the sum of
	 * the bytes times the index
	 * 
	...
	static int rollHash(int c) {
		...
		/*
		 * The original spamsum AND'ed this value with 0xFFFFFFFF which in
		 * theory should have no effect. This AND has been removed for
		 * performance (jk)
		 */
	...
	/*
	 * reset the state of the rolling hash and return the initial rolling hash
	 * value
	 */
	...
	static boolean ssInit(ss_context ctx, String handle) {
		if (ctx == null) {
			return true;
		}
	...
	static void ssEngine(ss_context ctx, byte[] buffer, int bufferSize) {
		...
		for (int i = 0; i < bufferSize; ++i) {
			 ...
			 * When the rolling hash hits the reset value then we emit the
			 * normal hash as a element of the signature and reset both hashes
			 */
			...
			if (((0xFFFFFFFFl & ctx.h) % ctx.block_size) == (ctx.block_size - 1)) {
				 ...
				 * on all chacaters in the piece of the message between the last
				...
				if (ctx.j < SPAMSUM_LENGTH - 1) {
					 ...
					 * easiest way to cope with this is to only reset the second
					 * hash if we have room for more characters in our
					 * signature. This has the effect of combining the last few
					 * pieces of the message into a single piece
					 */
			 ...
			 * block_size*2. By producing dual signatures in this way the effect
			 * of small changes in the message size near a block size boundary
			 * is greatly reduced.
			 */
	...
	static boolean ssUpdate(ss_context ctx, File handle) throws IOException {
		...
		// while ((bytes_read = fread(buffer,sizeof(byte),BUFFER_SIZE,handle)) >
		// 0)
	...
	static boolean ssUpdate(ss_context ctx, String handle, String filename)
			throws IOException {
		int bytesRead = 0;
		byte[] buffer;
	 ...
	 * @throws IOException
	 */
	public FuzzyHash fuzzyHashContext(String handle, String filename)
			throws IOException {
		ss_context ctx;
		boolean done = false;
		...
		ctx.ret.filename = filename;

		while (!done) {
			ssUpdate(ctx, handle, filename);
	...
	public FuzzyHash fuzzyHashBuf(byte[] buf, int bufLen) {
		...
		ctx.ret = new FuzzyHash(ctx.block_size, String.valueOf(ctx.p),
				String.valueOf(ctx.ret2));
	...
	/*
	 * we only accept a match if we have at least one common substring in the
	 * signature of length ROLLING_WINDOW. This dramatically drops the false
	 * positive rate for low score thresholds while having negligable affect on
	 * the rate of spam detection.
	 * 
	...
	static int hasCommonSubstring(char[] s1, char[] s2) {
		...
		/*
		 * there are many possible algorithms for common substring detection. In
		 * this case I am re-using the rolling hash code to act as a filter for
		 * possible substring matches
		 */

		rollReset();

		/*
		 * first compute the windowed rolling hash at each offset in the first
		 * string
		 */
		 ...
		 * compare it to all of the rolling hashes for the first string. If one
		 * matches then we have a candidate substring match. We then confirm
		 * that match with a direct string comparison
		 */
		/*
		 * for (i=0;i < s2.length;i++) { long h = roll_hash((char)s2[i]); if (i
		 * < ROLLING_WINDOW-1) continue; for
		 * (j=ROLLING_WINDOW-1;j<num_hashes;j++) { if (hashes[j] != 0 &&
		 * hashes[j] == h) { // we have a potential match - confirm it //FIXME
		 * if (strlen(s2+i-(ROLLING_WINDOW-1)) >= ROLLING_WINDOW &&
		 * strncmp(s2+i-(ROLLING_WINDOW-1), s1+j-(ROLLING_WINDOW-1),
		 * ROLLING_WINDOW) == 0) { return 1; }
		 * 
		 * } } }
		 */
	...
	/*
	 * this is the low level string scoring algorithm. It takes two strings and
	 * scores them on a scale of 0-100 where 0 is a terrible match and 100 is a
	 * great match. The block_size is used to cope with very small messages.
	 */
	static int scoreStrings(char[] s1, char[] s2, int blockSize) {
		...
		/*
		 * the two strings must have a common substring of length ROLLING_WINDOW
		 * to be candidates
		 */
		/*
		 ...
		 * gives us a pretty good idea of how closely related the two strings
		 * are
		 */
		score = StringUtils.getLevenshteinDistance(new String(s1), new String(
				s2));

		/*
		 * scale the edit distance by the lengths of the two strings. This
		 * changes the score to be a measure of the proportion of the message
		 * that has changed rather than an absolute quantity. It also copes with
		 * the variability of the string lengths.
		 */
		score = (score * SPAMSUM_LENGTH) / (len1 + len2);

		/*
		 * at this stage the score occurs roughly on a 0-64 scale, with 0 being
		 * a good match and 64 being a complete mismatch
		 */
		...
		/*
		 * it is possible to get a score above 100 here, but it is a really
		 * terrible match
		 */
		if (score >= 100) {
			return 0;
		...
		/*
		 * now re-scale on a 0-100 scale with 0 being a poor match and 100 being
		 * a excellent match.
		 */
		score = 100 - score;
		...
		/*
		 * when the blocksize is small we don't want to exaggerate the match
		 * size
		 */
	...
	/*
	 * given two spamsum strings return a value indicating the degree to which
	 * they match.
	 */
